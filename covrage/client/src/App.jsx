...
function App() {
  ...
  const [apiCoverageList, setApiCoverageList] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      console.log("this is previously stored data------------>", res.data);
      return res.data;
    } catch (error) {
      console.error(error);
    }
  ...
  const check = async (CAMS) => {
    try {
      const prevData = await fetchData(); // Wait for fetchData to complete
      if (!prevData) {
        throw new Error("Failed to fetch previous data");
      }

      const paths = prevData.map((data) => data.path);
      ...
      setData(coverage);
      console.log(`coverage list----> ${coverage} api list is ${apiList}`);

      const covered = coverage.filter((arr) => arr.covered).length;
      const notCovered = coverage.length - covered;
  ...
  useEffect(() => {
    if (prevdata.length === 0 || data.length === 0) return;

    let mergedArray = [];
    [prevdata, data].forEach((array) => {
      array.forEach((obj) => {
        let existingObj = mergedArray.find((o) => o.path === obj.path && o.method === obj.method);
        if (!existingObj) {
          mergedArray.push(obj);
        ...
        <div className='w-[98vw] overflow-hidden'>
          <div className='flex flex-col'>
            ...
            <div className='bg-slate-400 flex justify-evenly'>
              ...
              <button className='bg-green-500 font-semibold rounded-lg items-end p-3 my-3' onClick={retrive}>Click to extract Checked Api</button>
            </div>
          </div>
          {showCharts && (
